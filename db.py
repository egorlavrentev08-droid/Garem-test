    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                last_inactive_notification TEXT
            )
        """)
...
async def set_shield_until(user_id: int, hours: int):
    """Активирует щит на N часов (с учётом тестового режима)"""
    async with aiosqlite.connect(DB_PATH) as db:
        if is_test_mode():
            await db.execute(
                "UPDATE users SET shield_until = datetime('now', '+' || ? || ' seconds') WHERE user_id = ?",
                (hours, user_id)
            )
        else:
            await db.execute(
...
async def reset_daily_messages():
    """Сбрасывает счётчик сообщений и флаг начисления стрика в 00:00"""
    async with aiosqlite.connect(DB_PATH) as db:
...
async def get_top_streak(limit: int = 15) -> List[Dict]:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        for row in rows:
            data = dict(row)
            if not data.get('name') or data['name'] == 'Без имени':
                data['display_name'] = data.get('telegram_username') or str(data['user_id'])
            else:
...
async def start_redemption(user_id: int, lost_streak: int):
    """Активирует искупление (с учётом тестового режима)"""
    if is_test_mode():
        expires_at = datetime.now() + timedelta(seconds=REDEMPTION_HOURS)
    else:
...
async def get_active_redemptions() -> List[Dict]:
    """Возвращает пользователей с активным искуплением"""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute("""
            SELECT user_id, name, telegram_username, 
                   redemption_progress, redemption_target,
                   redemption_expires_at
            FROM users
            WHERE redemption_active = 1
        """)
...
async def award_daily_top() -> List[Dict]:
    """Награждает топ-3 по сообщениям за сегодня"""
...
async def get_inactive_users() -> List[Dict]:
    """Возвращает пользователей, которые не писали > 24 часов (с учётом тестового режима)"""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        ...
        if is_test_mode():
            cursor = await db.execute("""
                ...
                FROM users
                WHERE last_activity < datetime('now', '-24 seconds')
                  AND (shield_until IS NULL OR shield_until < datetime('now'))
                  AND redemption_active = 0
            """)
        else:
            cursor = await db.execute("""
                SELECT user_id, name, telegram_username, last_activity, streak
...
# ============================================================
# 10. УВЕДОМЛЕНИЯ О НЕАКТИВЕ
# ============================================================

async def update_inactive_notification(user_id: int):
    """Обновляет время последнего уведомления о неактиве"""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE users SET last_inactive_notification = datetime('now') WHERE user_id = ?",
            (user_id,)
        )
        await db.commit()


async def get_inactive_notification_time(user_id: int) -> Optional[datetime]:
    """Возвращает время последнего уведомления"""
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("SELECT last_inactive_notification FROM users WHERE user_id = ?", (user_id,))
        row = await cursor.fetchone()
        if row and row[0]:
