    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                redemption_target INTEGER DEFAULT 4,
...
async def start_redemption(user_id: int, lost_streak: int):
    if is_test_mode():
        expires_at = datetime.now() + timedelta(seconds=24)
    else:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if row:
            return {
                ...
                'target': row['redemption_target'] if row['redemption_target'] is not None else 4,
