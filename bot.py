...
async def cmd_redemption(message: types.Message):
    ...
    try:
        ...
        hours_left = (expires - datetime.now()).total_seconds()
    except:
        hours_left = 24
    ...
    text = (
        ...
        f"⏳ Осталось: ~{int(hours_left)} секунд\n\n"
        f"{'🔥 ДАВАЙ, ТЫ СМОЖЕШЬ!' if progress > 2 else '💪 ПИШИ БОЛЬШЕ!'}"
    )
...
async def cmd_admin_ban(message: types.Message):
    ...
    if success:
        ...
        name = user.get('name', user.get('telegram_username', str(user_id)))
