...
from config import (
    ...
    get_rank_by_streak, DB_PATH, PICS_PATH
)
from db import (
    get_user, register_user, update_last_activity, add_coins,
    get_streak, increment_streak, update_streak,
    increment_messages_today,
    get_shield_count, is_shield_active, use_shield, set_shield_until, deactivate_shield, add_shield,
    ...
    add_reward_history,
    update_inactive_notification, get_inactive_notification_time
)
...
def load_phrases():
    ...
    if not os.path.exists(PHRASES_FILE):
        logger.warning(f"⚠️ Файл {PHRASES_FILE} не найден")
        _phrase_cache = {}
        return
...
async def process_user_message(user_id: int, chat_id: int) -> Dict:
    ...
    # 4. Проверяем пропуск дня
    last_activity = user.get('last_activity')
    if last_activity:
        try:
            last_time = datetime.fromisoformat(last_activity)
            hours_since = get_time_since_hours(last_time)
    ...
    # ============================================================
    # 5. СТРИК РАСТЁТ ТОЛЬКО ЕСЛИ:
    #    - streak_awarded_today == 0 (сегодня ещё не начисляли)
    #    - messages_today >= 100 (написано 100+ сообщений)
    #    - last_activity существует (не первое сообщение в жизни)
    # ============================================================
    ...
    # 9. Ежедневное приветствие (в чат!)
    if should_increment:
...
async def check_inactive_users() -> List[Dict]:
    """Проверяет неактивных пользователей с расписанием:
       - 24 часа: первое ANGRY уведомление
       - 24-48 часов: ANGRY каждые 3-5 часов
       - 48+ часов: FURIOUS каждые 3-5 часов
    """
    ...
    for user in inactive:
        ...
        hours_since = get_time_since_hours(datetime.fromisoformat(user['last_activity']))
        last_notif = await get_inactive_notification_time(user['user_id'])
        
        # 1. Первое уведомление через 24 часа (ровно)
        if 24 <= hours_since < 24.5:
            result.append({
                'user': user,
                'type': 'first',
        ...
        # 2. Если прошло 48+ часов — FURIOUS каждые 3-5 часов
        if hours_since >= 48:
            if not last_notif:
                result.append({
                    'user': user,
                ...
                hours_since_last = get_time_since_hours(last_notif)
                if hours_since_last >= random.uniform(3, 5):
                    result.append({
                        'user': user,
                        'type': 'aggressive',
                        'trigger': 'MULTI_DAY_INACTIVE',
        ...
        # 3. Если прошло 24-48 часов — ANGRY каждые 3-5 часов
...
async def get_top_streak_text(limit: int = 15) -> str:
    ...
    for i, user in enumerate(top):
        ...
        name = user.get('display_name', user.get('name', user.get('telegram_username', str(user['user_id']))))
...
async def handle_inactive_user(user_id: int, trigger: str, mood: str) -> Dict:
    """Обрабатывает неактивного пользователя и обновляет время уведомления"""
    user = await get_user(user_id)
    if not user:
    ...
    redemption = await get_redemption_status(user_id)
    is_redeeming = redemption and redemption.get('active', False)
    
    # Только если нет искупления - сбрасываем стрик
    if not is_redeeming:
        old_streak = user['streak']
        if old_streak > 0:
            await update_streak(user_id, 0)
        ...
        # Если уже в искуплении - просто напоминаем
        text = "⚠️ Ты всё ещё в искуплении! Пиши сообщения!"
    ...
    # Добавляем информацию об искуплении
    if redemption:
        progress = redemption.get('progress', 0)
        target = redemption.get('target', REDEMPTION_TARGET)
        text += f"\n\n🔄 **Восстановление стрика:**\n{progress}/{target} сообщений\n\nПрогресс: **/redemption**"
    
    # Обновляем время уведомления
    await update_inactive_notification(user_id)
    
    # 🔥 ВАЖНО: Обновляем активность, чтобы убрать из списка неактивных
    await update_last_activity(user_id)
