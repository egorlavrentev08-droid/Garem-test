...
from config import (
    ...
    get_rank_by_streak, DB_PATH, PICS_PATH, MEMS_PATH, CHAT_ID
)
from db import (
...
async def process_user_message(user_id: int, chat_id: int) -> Dict:
    ...
    if last_activity and is_main_chat:
        try:
            ...
            h_ago = 168 if is_test_mode() else 7 * 24 * 3600
            
            if seconds_since >= h_ago:
    ...
    if is_main_chat:
        ...
        if last_activity and streak_awarded_today == 0 and messages_today >= 2:
...
async def check_inactive_users() -> List[Dict]:
    ...
    for user in inactive:
        ...
        if seconds_since >= 2592000:
            if not await is_user_banned(user['user_id']):
                await ban_user(user['user_id'])
                logger.info(f"🚫 Автобан: {user['user_id']} за месяц неактива")
            continue
        
        h24 = 24 if is_test_mode() else 24 * 3600
        h48 = 48 if is_test_mode() else 48 * 3600
        h_ultra = 168 if is_test_mode() else 7 * 24 * 3600
        h3 = 3 if is_test_mode() else 3 * 3600
...
async def get_user_profile(user_id: int) -> Optional[str]:
    ...
    if redemption and redemption.get('active', False):
        ...
        target = redemption.get('target', 4)
...
async def handle_inactive_user(user_id: int, trigger: str, mood: str) -> Dict:
    ...
    if seconds_since >= h48:
        ...
        if is_already_redeeming:
            ...
            target = redemption.get('target', 4) if redemption else 4
            remaining = target - progress
