...
async def scheduler_loop(bot: Bot):
    ...
    last_inactive_check = datetime.now() - timedelta(seconds=10)
    last_daily_reset = datetime.now() - timedelta(seconds=30)
    last_weekly_reset = datetime.now() - timedelta(seconds=200)
    last_redemption_check = datetime.now() - timedelta(seconds=10)
    
    while True:
        try:
            now = datetime.now()
            
            if (now - last_inactive_check).total_seconds() >= 5:
            ...
            if (now - last_redemption_check).total_seconds() >= 5:
            ...
            try:
                ...
                if "сброшен" in result['text']:
                    ...
                    await bot.send_message(
                        ...
                        f"Но может восстановить — {200} сообщений за 24 секунды. 👀"
                    )
            except Exception as e:
